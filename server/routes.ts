...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use(express.static(path.join(process.cwd()), {
    setHeaders: (res, filePath) => {
      ...
      // Разрешаем доступ к booomerangs-smart-chat.html
      if (filePath.includes('booomerangs-smart-chat.html')) {
        return true;
      }
      // Блокируем другие HTML файлы чатов чтобы не было конфликтов
  ...
  app.get('/optimized', (req, res) => {
    res.sendFile('booomerangs-optimized-chat.html', { root: '.' });
  });
  ...
  app.post('/api/chat/sessions/:sessionId/messages', async (req, res) => {
    try {
      ...
      // Быстрое обновление заголовка только для первого сообщения
      if (messageData.sender === 'user' && messageData.content) {
        // Асинхронно обновляем заголовок без ожидания
        setImmediate(async () => {
          try {
            ...
            console.log(`🎯 Заголовок обновлен: "${smartTitle}"`);
          } catch (titleError) {
            console.log('⚠️ Ошибка обновления заголовка:', titleError);
          }
        });
      }
    ...
    try {
      ...
      try {
        ...
        if (data.text && data.text.trim()) {
          ...
          return res.json({
            success: true,
            text: `📄 **Анализ PDF документа "${file.originalname}"**\n\nЭтот PDF файл содержит графические элементы (изображения, логотипы) вместо текстовых данных.\n\n📊 **Информация о файле:**\n- Страниц: ${data.numpages || 1}\n- Размер: ${Math.round(dataBuffer.length / 1024)} КБ\n- Тип: Графический PDF\n\n💡 **Рекомендация:** Для анализа изображений используйте форматы JPG, PNG или загрузите PDF с текстовым содержимым.`,
