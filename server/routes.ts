...
*🚀 Анализ выполнен с помощью бесплатных AI сервисов!*`;
  ...
  app.post("/api/stream", async (req, res) => {
    ...
    console.log(`🔧 [DEBUG] Запрошенный провайдер: "${requestedProvider}"`);
    console.log(`🔧 [DEBUG] Наш провайдер: "${provider}"`);
    ...
    // 🧪 АВТОМАТИЧЕСКАЯ ПРОВЕРКА РАБОТЫ КОНТЕКСТА
    console.log('🔍 [AUTO-CHECK] Проверяем работу контекста...');
    if (contextInfo.context && contextInfo.context.trim()) {
      console.log('✅ [AUTO-CHECK] КОНТЕКСТ РАБОТАЕТ: найден контекст длиной', contextInfo.context.length, 'символов');
      console.log('📝 [AUTO-CHECK] Предыдущие сообщения:', contextInfo.messageHistory?.length || 0);
    } else {
      console.log('⚠️ [AUTO-CHECK] КОНТЕКСТ ПУСТОЙ - это первое сообщение или ошибка памяти');
    }
    
    // Проверяем вопросы о предыдущих сообщениях
    const memoryQuestions = ['как меня зовут', 'что я говорил', 'помнишь', 'моё имя', 'из какого города'];
    const isMemoryQuestion = memoryQuestions.some(q => message.toLowerCase().includes(q));
    if (isMemoryQuestion) {
      console.log('🧠 [AUTO-CHECK] ОБНАРУЖЕН ВОПРОС О ПАМЯТИ!');
      if (contextInfo.context && contextInfo.context.trim()) {
        console.log('✅ [AUTO-CHECK] ПАМЯТЬ РАБОТАЕТ: есть контекст для ответа');
      } else {
        console.log('❌ [AUTO-CHECK] ПАМЯТЬ НЕ РАБОТАЕТ: нет контекста для ответа на вопрос о прошлом');
      }
    }
