  ...
  app.post("/api/stream", async (req, res) => {
    ...
    if (sessionId) {
      ...
      try {
        ...
        // Проверяем что сохранилось (временно отключено для отладки)
    ...
    try {
      ...
      // Формируем подробную информацию для AI с контекстом
      if (searchResults.length > 0) {
        ...
        if (sessionId) {
          ...
          try {
            ...
            console.log('💭 [CONTEXT] ⚠️ Контекст временно отключен для отладки');
        ...
        searchInfo += `\nИНСТРУКЦИЯ ДЛЯ AI: Используй ТОЛЬКО эту актуальную информацию из интернета. НЕ давай общие ссылки на Яндекс.Карты или 2ГИС. Предоставь пользователю конкретные названия магазинов, адреса и контакты из найденных результатов. Учитывай предыдущий контекст разговора.\n\n`;
          ...
          if (data.response && typeof data.response === 'string') {
            ...
            if (sessionId) {
              try {
                ...
                // Проверяем общее количество сообщений в контексте (временно отключено)
