...
async function getAIResponseWithSearch(userQuery) {
  try {
    ...
    const prompt = `Ты - AI ассистент с расширенными возможностями:

ТВОИ ВОЗМОЖНОСТИ:
- Генерация изображений (ключевые слова: нарисуй, создай изображение, сгенерируй картинку, принт, дизайн)
- Веб-поиск для актуальной информации (погода, новости, курсы валют, время работы)
- Анализ изображений
...
ПРАВИЛА ОТВЕТА:
1. Если это запрос на ГЕНЕРАЦИЮ ИЗОБРАЖЕНИЯ (нарисуй, создай, сгенерируй) - ответь: "ГЕНЕРАЦИЯ_ИЗОБРАЖЕНИЯ"
2. Если это запрос о текущей информации (погода, новости, курсы) - ответь: "НУЖЕН_ПОИСК"  
