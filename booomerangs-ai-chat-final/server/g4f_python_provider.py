...
def chat():
    ...
    try:
        ...
        is_shop_request = any(word in message.lower() for word in ['магазин', 'одежда', 'торговый', 'тула', 'ищу', 'найди'])
        has_search_data = "🔍 **АКТУАЛЬНАЯ ИНФОРМАЦИЯ ИЗ ИНТЕРНЕТА:**" in message
        
        print(f"🔥🔥🔥 [PYTHON G4F] Запрос о магазинах: {is_shop_request}")
        print(f"🔥🔥🔥 [PYTHON G4F] Обнаружены поисковые данные: {has_search_data}")
        
        # Если это запрос о магазинах, ВСЕГДА используем настоящий AI
        if is_shop_request:
            print(f"🔥🔥🔥 [PYTHON G4F] ЗАПРОС О МАГАЗИНАХ - ИСПОЛЬЗУЕМ НАСТОЯЩИЙ AI!")
        elif not has_search_data:
            # Только для остальных запросов без поисковых данных используем демо-режим
