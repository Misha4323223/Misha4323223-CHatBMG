...
// Сохраняем HTML файл
fs.writeFileSync(path.join(publicDir, 'generator.html'), htmlPage);
...
// Маршрут для отображения генератора изображений
app.get('/svg-generator', (req, res) => {
...
// Перенаправляем запросы к Python G4F API
app.post('/api/python/g4f/chat', async (req, res) => {
    try {
        ...
        console.error('Ошибка при обращении к G4F API:', error);
        res.status(500).json({
            error: 'Ошибка при обращении к G4F API',
            message: error.message,
            response: 'К сожалению, не удалось получить ответ от G4F. Пожалуйста, попробуйте еще раз позже.',
            provider: 'error',
            model: 'none'
        });
    }
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`Откройте http://localhost:${PORT}/image-generator в браузере`);
});